...
pub fn resolve_integration_test_path(path: impl AsRef<std::path::Path>) -> std::path::PathBuf {
    if path.as_ref().is_relative() {
        if let Ok(integration_tests_env_var) = std::env::var("CEDAR_INTEGRATION_TESTS_PATH") {
            return std::path::PathBuf::from(integration_tests_env_var);
        }
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR")
            .expect("`CARGO_MANIFEST_DIR` should be set by Cargo at build-time.");
        let mut full_path = std::path::PathBuf::from(manifest_dir.clone());
        full_path.push("..");
